...
{  
    ...
    for (i = 0; i < old_size; i++){  
		...
		status = em_hashmap_put(m, elem->key, elem->data);  
		if (status != HMAP_S_OK) {  
			return status;  
		}  
    }  
...
int em_hashmap_put(hmap_t in, char* key, void_ptr value)
{  
	int index;  
	hashmap_map_t *m;  
	hashmap_elem_t *elem;  
...
int em_hashmap_get(hmap_t in, const char* key, void_ptr *value)
{  
	int curr;  
	int i;  
	hashmap_map_t *m;  
...
int em_hashmap_iterate(hmap_t in, hmap_callback_func fnIterValue, void_ptr arg) 
{  
    int i;  
    hashmap_elem_t *elem;  
    hashmap_map_t *m = (hashmap_map_t*) in;  
    
    if (em_hashmap_size(m) <= 0) {  
      return HMAP_E_NOTFOUND;  
    }  
...
int em_hashmap_remove(hmap_t in, char* key, void_ptr *outValue)
{  
	int i, curr;  
	hashmap_map_t* m;  
	hashmap_elem_t *elem;  
...
void em_hashmap_destroy(hmap_t in, hmap_callback_func fnFreeValue, void_ptr arg)
{  
	hashmap_elem_t *elem;  
	void_ptr data;  
	hashmap_map_t* m = (hashmap_map_t*) in;  
